    ...
    def load(self):
        ...
        for job_name, create_time in abort_jobs:
            ...
            try:
                ...
                pa = self.running_jobs.get(job_name,None)
                if pa: #
                    pid = pa[0].pid
                    ji = pa[1]
                    #p.terminate()
        ...
        for job_name,create_time in new_jobs:
            ...
            if not self.running_jobs.has_key(job_name):  #if this job was not running
                ...
                myjobs =  self.user_jobs.get(job_owner) #check out how many jobs are running under this user
                if myjobs: 
        ...
        available_nodes = self.util.NUM_CLUSTER_NODES_AVAILABLE - len(self.running_jobs)
        _job_rank = 0
        ...
        for k in range(0,available_nodes):
            if not pending_jobs.empty():
                ...
                p.start()
                self.running_jobs[job_name] = (p,self.job_index)
    
    def _sync(self):
        self.util.sync_get_app()
        self.util.sync_get_data()
        self.util.sync_get_conf()
